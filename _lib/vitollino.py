...
class Popup:
    ...
    @staticmethod
    def d(cena, tit="", txt=""):
        _ = cena.elt <= Pop().popup
        _ = cena.elt <= Pop().go
        act = cena.vai
        cena.vai = lambda *_, **__: Pop().mostra(act, tit, txt)
        return cena
...
class Texto(Popup):
    ...
    def mostra(self, tit="", txt="", act=None, **kwargs):
        ...
        Pop().esconde = self.esconde
        Pop().mostra(act, tit=tit, txt=txt, **kwargs)

    def vai(self, ev=NoEv()):
        ev.stopPropagation()
        # self.cena.elt <= Popup.__setup__().popup
        _ = self.cena.elt <= Pop().popup
